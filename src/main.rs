use std::{
    alloc::{self, Layout}, io::Read, ptr::{self, null, null_mut}
};
...
fn SetHardwareBreakpoint(address_of_breakpoint: PVOID) -> bool {
    unsafe {
        ...
        let mut ctx = alloc::alloc(layout) as *mut CONTEXT;
        (*ctx).ContextFlags = CONTEXT_DEBUG_REGISTERS;
        let currentThread: HANDLE = GetCurrentThread();
        let status = GetThreadContext(currentThread, ctx);
        if status == FALSE {
            println!("Error getting thread context: {}", GetLastError());
            return false;
        }
        ...
        if (SetThreadContext(currentThread, ctx) == FALSE) {
